                ...
                <form action="{{ route('users.index') }}" method="GET" class="space-y-6">
                    ...
                    <div class="flex flex-col md:flex-row gap-4 items-end">
                        ...
                        <div class="flex-1 w-full">
                            <label class="block mb-2 text-xs font-semibold tracking-normal text-gray-500">Cari Pengguna</label>
                            <div class="relative group">
                                ...
                                <input type="text" name="search" value="{{ request('search') }}"
                                       class="bg-gray-50 border border-gray-200 text-sm rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-full ps-10 p-3 transition"
                        ...
                        <div class="w-full md:w-32">
                            <label class="block mb-2 text-xs font-semibold tracking-normal text-gray-500">Tampilkan</label>
                            <select name="limit" class="bg-gray-50 border border-gray-200 text-sm rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-full p-3 transition">
                        ...
                        <button type="submit" class="w-full md:w-auto bg-emerald-600 text-white px-8 py-3 rounded-xl text-sm font-bold hover:bg-emerald-700 transition shadow-lg shadow-emerald-100 active:scale-95">
                            Terapkan Filter
                        </button>
                    </div>
                    ...
                    <div class="pt-4 border-t border-gray-100">
                        <span class="block mb-3 text-xs font-semibold tracking-normal text-gray-500">Filter Role:</span>
                            ...
                            @foreach(['Mahasiswa', 'Dosen', 'Laboran', 'Aslab'] as $roleName)
                                <label class="inline-flex items-center cursor-pointer group">
                                    <input type="checkbox" name="roles[]" value="{{ $roleName }}"
                                           ...
                                           class="w-5 h-5 text-emerald-600 bg-gray-50 border-gray-200 rounded-lg focus:ring-emerald-500 transition">
                                    <div class="ms-3 flex flex-col">
                                        <span class="text-sm font-bold text-gray-700 group-hover:text-emerald-600 transition">{{ $roleName }}</span>
                                        @if($roleName === 'Mahasiswa')
                                            <span class="text-xs text-gray-400 font-medium leading-none italic">Termasuk Aslab</span>
                                        @endif
                                    </div>
                                </label>
                            @endforeach
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-2xl border border-gray-200">
                <div class="overflow-x-auto">
                    <table class="min-w-[900px] w-full text-sm text-left">
                        <thead class="text-xs font-semibold tracking-normal text-gray-400 bg-gray-50/50 border-b border-gray-100">
                            <tr>
                                ...
                                <tr class="hover:bg-gray-50/50 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex justify-center">
                                            ...
                                            @else
                                                <div class="w-12 h-12 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-700 font-semibold border border-emerald-100">
                                    ...
                                    {{-- ID / NIM --}}
                                    <td class="px-6 py-4">
                                        <span class="text-xs font-semibold tracking-tighter text-emerald-700">{{ $user->id }}</span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <span class="px-3 py-1.5 rounded-xl text-xs font-semibold tracking-normal
                                            {{ $user->role == 'Mahasiswa' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' :
                                               ($user->role == 'Dosen' ? 'bg-slate-100 text-slate-700 border border-slate-200' :
                                               ($user->role == 'Laboran' ? 'bg-amber-50 text-amber-700 border border-amber-100' : 'bg-slate-50 text-slate-600 border border-slate-200')) }}">
                                            {{ $user->role }}
                                        </span>
                                    </td>
