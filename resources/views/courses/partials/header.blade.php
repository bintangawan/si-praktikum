@if($course->isArchived())<p class="mb-5 rounded-xl bg-amber-50 p-4 text-sm text-amber-800">Kelas arsip hanya dapat dibaca.</p>@endif
{{-- Header & Navigasi --}}
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
    <a href="{{ route($course->isArchived() ? 'archives.index' : 'courses.index') }}" class="text-emerald-600 hover:text-emerald-800 text-sm font-semibold flex items-center transition group">
    ...
    @if(!$course->isArchived() && in_array(strtoupper(Auth::user()->role), ['ASLAB', 'LABORAN']))
